...
func shrinkToContentSidePanelBoxes(args WindowArrangementArgs, windows []string, height int) ([]*boxlayout.Box, bool) {
	...
	// scroll; share the leftover among all of them, weighting toward the focused
	// panel in accordion mode. Deal out the rounding remainder one row at a time
	// so the heights fill the available space exactly.
	for _, p := range flexible {
		p.height = p.desired
	}
	totalWeight := 0
	...
	leftover := surplus
	for _, p := range flexible {
		share := surplus * p.weight / totalWeight
		p.height += share
		leftover -= share
	}
	for _, p := range flexible {
